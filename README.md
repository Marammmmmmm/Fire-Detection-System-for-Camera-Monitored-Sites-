...
2. `+ Add new secret`, name it `ROBOFLOW_KEY`, paste your key
3. Enable **Notebook access**

### Reproducing the results

1. Open `02_Evaluation_Testing_and_Demo.ipynb`
2. Run sections 1–3 (environment, dataset, Drive)
3. Set `WEIGHTS` in section 5 to `model/run1_20ep.pt` and run it — this reproduces
   the 0.710 mAP figure
4. Continue through the notebook for calibration, unseen-image tests and the demo
...
1. **Smoke detection does not work.** At its best operating point half the alarms
   ...
   reported in full.
2. **Night-time failure.** One unseen night scene produced no detection at all. The
...
4. **Test metrics are optimistic** because of the augmentation leakage described
   above.
